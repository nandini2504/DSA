...
struct Node* TBST::caseA(struct Node* root, struct Node* par, struct Node* ptr) {
    if (par == NULL)
        root = NULL;
    ...
    free(ptr); // Change this to delete ptr;
    return root;
}

struct Node* TBST::caseB(struct Node* root, struct Node* par, struct Node* ptr) {
    struct Node* child;
    if (ptr->lthread == 1)
        child = ptr->left;
    else
    ...
    delete ptr; // Change this to delete ptr;
    return root;
}

// Here 'par' is pointer to parent Node and 'ptr' is
// pointer to current Node.
struct Node* TBST::caseC(struct Node* root, struct Node* par, struct Node* ptr) {
